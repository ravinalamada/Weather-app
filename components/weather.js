          ...
          <div className="weathers--container">
            ...
            <div className="weathers--wrapper">
              ...
              <div key={weather.id} className="weather--wrapper--contents">
                <p className="date">{weather.applicable_date === dateTom ? 'Tommorow' : new Date(weather.applicable_date).toLocaleDateString('en-us', { day: 'numeric', weekday: 'short', month: 'short' })}</p>
                ...
                <div className="weather--temp--wrapper">
                  {isCeluis ? <p className="date"> {Math.floor((weather.max_temp) * 1.8 + 32)}  &deg;F</p>: <p>{Math.floor(weather.max_temp)}  &deg;C</p>}
                  {isCeluis ? <p className="date"> {Math.floor((weather.min_temp) * 1.8 + 32)}  &deg;F</p>: <p>{Math.floor(weather.min_temp)}  &deg;C</p>}
                </div>
              </div>
            ))}
            </div>
