        ...
        <WeatherLoading />
        {
          ...
          <div className="weather--wrapper">
            <div className="temp--units--wrapper">
              <button className={ `${!isCeluis ? 'bg-light-gray' : 'bg-dark-gray'} weather__btn`} onClick={convertFarUnitIntoCeluis}>&deg;C</button>
              <button className= {isCeluis ? 'bg-light-gray' : 'bg-dark-gray'} onClick={convertUnitCeluisIntoFar}>&deg;F</button>
            </div>
            <div className="weather--subWrapper">
              ...
              <div key={weather.id} className="weather--wrapper--contents">
                <p className="weather--date">{weather.applicable_date === dateTom ? 'Tommorow' : new Date(weather.applicable_date).toLocaleDateString('en-us', { day: 'numeric', weekday: 'short', month: 'short' })}</p>
                <img className="weather--images" src={`https://www.metaweather.com//static/img/weather/${weather.weather_state_abbr}.svg`} alt="photo"/>
                <div className="weather--temp--wrapper">
                  {isCeluis ? <p className="weather--unit"> {Math.floor((weather.max_temp) * 1.8 + 32)}  &deg;F</p>: <p>{Math.floor(weather.max_temp)}  &deg;C</p>}
                  {isCeluis ? <p className="weather--unit"> {Math.floor((weather.min_temp) * 1.8 + 32)}  &deg;F</p>: <p>{Math.floor(weather.min_temp)}  &deg;C</p>}
                </div>
              </div>
            ))}
            </div>
            <div>
              ...
              <div className="weather--highlight--container">
                <div className="weather--highlight--wrapper">
                  ...
                  <p className='weather--direction'>
                    <span className='weather--direction--value'>{weatherToday.wind_speed.toFixed(2)}</span>
                    mph
                  </p>
                ...
                <div className="weather--highlight--wrapper">
                  ...
                  <p className="weather--humidity">
                    <span className='weather--humidity--value'>79</span>
                    %
                  </p>
                  <div className='progress--wrapper'>
                    ...
                    <div className="progress__image--wrapper">
                      <div className='progress-bar' style={{width: weatherToday.humidity}}>
                      </div>
